# Hi there! 👋 I'm Felipe H. Vidal

## Security Professional & Future IT Specialist (Fachinformatiker für Systemintegration Candidate)

Retired Captain with a ZAB-recognized Bachelor's Degree in Public Security Administration. Transitioning my analytical mindset, risk assessment experience, and strong teamwork skills under high-pressure situations into IT infrastructure and systems integration.

Currently leveraging my parental leave to build a solid, self-taught foundation in Hardware, Operating Systems, Networking, and IT Fundamentals, actively preparing to start a dual vocational training (*Ausbildung zum Fachinformatiker für Systemintegration*) in August 2027.

---

### 🛠️ What I am working on:
* 💻 **Systems & Hardening:** Documenting OS optimization, security configurations, and local system management (Windows & Linux).
* 🔧 **Hardware & Maintenance:** Explaining physical lifecycles, thermal management, and preventative maintenance.
* 📚 **Self-Taught Roadmap:** Advancing modularly through AI basics, corporate networking fundamentals (TCP/IP, routing, switching), and IT support essentials.

---

### 🎯 My Goals:
* 🎓 **Vocational Training 2027:** Secure an *Ausbildung* position as *Fachinformatiker für Systemintegration* in a forward-thinking company.
* 📜 **Foundational IT Credentials:** Achieve industry certifications to validate my hands-on skills (such as CompTIA A+ and CompTIA Network+).
* 🇩🇪 **Technical & Language Competencies:** Further enhance my technical German (B2/C1) and build strong technical English skills through practical lab documentation.
